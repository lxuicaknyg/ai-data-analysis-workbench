    ...
    <div class="indicator-summary" :class="{ expanded }">
      <NFlex align="center" :size="6" style="flex: 1; min-width: 0;">
        <NFlex vertical :size="2" style="min-width: 0;">
          ...
          <NText depth="3" style="font-size: 11px;">单位：{{ indicator.unit }}</NText>
        </NFlex>
      </NFlex>

      <NFlex align="center" :size="6" style="flex-shrink: 0;">
        <!-- 必填/可选 badge -->
        <NTag size="small" :type="indicator.required ? 'error' : 'default'" :bordered="false">
          {{ indicator.required ? '必填' : '可选' }}
        </NTag>
    ...
    <Transition name="sql-slide">
      <div v-if="expanded" class="sql-editor-area">
        <NFlex vertical :size="10">
          <!-- 指标定义说明（来自后端，帮助用户理解该指标） -->
          <div v-if="indicator.description" class="indicator-description">
            <NText style="font-size: 11px; color: var(--n-text-color-3);">指标定义：</NText>
          ...
          <div class="sql-editor-box">
            <NText class="sql-editor-hint">查询语句（{{ draftDescription || `返回单一数值，单位：${indicator.unit}` }}）</NText>
            <NInput
          ...
          <div v-if="indicator.sqlConfig?.testStatus === 'success'" class="test-result-block">
            ...
            <div v-if="isSingleColumnList" class="result-preview">
              <NText depth="3" style="font-size: 11px;">
                查询返回 {{ indicator.sqlConfig.testRowCount }} 行：
              </NText>
              <div class="result-list">
                <NText
                  v-for="(row, i) in indicator.sqlConfig.testRows"
                  :key="i"
            ...
            <div v-if="isTable" class="result-preview">
              ...
              />
              <NText v-if="(indicator.sqlConfig.testRowCount ?? 0) > 5" depth="3" style="font-size: 11px; display: block; margin-top: 4px;">
                仅显示前 5 行
              </NText>
            </div>
          </div>
...
import type { Indicator } from './reportTypes'
